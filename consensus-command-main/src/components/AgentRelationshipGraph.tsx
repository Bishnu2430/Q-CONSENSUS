...
function computeForceLayout(
  ...
  const nodes: SimNode[] = agentIds.map((id, i) => ({
    id,
    x: WIDTH / 2 + Math.cos((i / agentIds.length) * Math.PI * 2) * 110,
    y: HEIGHT / 2 + Math.sin((i / agentIds.length) * Math.PI * 2) * 110,
    vx: 0,
    vy: 0,
  }));
  ...
  const sim = forceSimulation(nodes)
    .force(
      ...
      forceLink<SimNode, LayoutEdge & { source: string | SimNode; target: string | SimNode }>(simEdges)
        .id((d) => d.id)
        .distance((d) => 210 - (d as unknown as LayoutEdge).weight * 130)
        .strength((d) => (d as unknown as LayoutEdge).weight * 0.6),
    )
  ...
  for (const node of nodes) {
    next[node.id] = {
      x: Math.max(44, Math.min(WIDTH - 44, node.x)),
      y: Math.max(44, Math.min(HEIGHT - 44, node.y)),
    };
  }
  return next;
}
...
export function AgentRelationshipGraph() {
  ...
  const positions = useForceLayout(snapshot?.agentIds ?? [], edges);
          ...
          >
            ...
            <g>
              {edges.map((edge, i) => {
                ...
                const dimmed =
                  selectedAgent && selectedAgent !== edge.source && selectedAgent !== edge.target;
                return (
                  <motion.path
                    key={i}
                    d={edgePath(from, to)}
                    ...
                    animate={{
                      pathLength: 1,
                      opacity: dimmed ? 0.04 : 0.14 + edge.weight * 0.45,
                    }}
